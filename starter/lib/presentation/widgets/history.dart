...
class MessageHistory extends StatefulWidget {
  const MessageHistory(this.cubit, this.state, {required this.layout, super.key});
...
class _MessageHistoryState extends State<MessageHistory> {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleHistory = isMobile
        ...
        : widget.state.history.skip(_currentPage * _pageSize).take(_pageSize).toList();

    return SectionCard(
      title: 'Message history',
      trailing: Text(
        isMobile ? '${visibleHistory.length} recent' : 'Page ${_currentPage + 1}',
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.state.history.isEmpty && widget.state.error == null)
            ...
            if (isMobile) ...[
              if (widget.state.history.length > _pageSize || widget.state.nextCursor != null)
                Padding(
                  ...
                  child: OutlinedButton(
                    onPressed: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (ctx) => const MobileHistoryPage(),
                        ),
                      );
                    },
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      ...List.generate(
                        ((widget.state.history.length - 1) ~/ _pageSize) + 1 + (widget.state.nextCursor != null ? 1 : 0),
                        (index) {
                          ...
                          final isLoadingThisPage = index > ((widget.state.history.length - 1) ~/ _pageSize) && widget.state.loadingMore;
                      ...
                      IconButton(
                        ...
                        onPressed: ((widget.state.history.length - 1) ~/ _pageSize) > _currentPage || widget.state.nextCursor != null
